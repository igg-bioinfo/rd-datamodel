...
class Lookup:
    ...
    def save_setup(self, text):
        ...
        text_base = file.read()
        file.close()
        text_base = text_base.replace("dist/umdm.xlsx", "dist/EUROFEVER.xlsx")
        #text_base = text_base.replace("umdm_", "efm_")
        text_base = text_base.split(eof)[0] + text + eof
        with open(self.setup_file, "w") as f:
            f.write(text_base)

    def save_csv(self, name, options):
        file_name = "efm_lookups_" + name + ".csv"
        print("Save " + file_name)
        text = "value,description\n"
        for opt in options:
            text += opt[0] + "," + opt[1] + "\n"
        with open(os.path.join(self.lookup_path, file_name), "a") as f:
            f.write(text)
    ...
    def save(self, lookups):
        ...
        for lu in lookups:
            txt_setup += "mcmd import -p lookups/efm_lookups_" + lu[0] + ".csv\n"
            txt_yaml += self.add(lu[0], "Eurofever lookup")
    ...
    def add(self, name, description):
        ...
        yaml += "    extends: efm_lookups_" + self.template + "\n"
        return yaml
