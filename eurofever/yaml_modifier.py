...
def main(argv):
    ...
    if os.path.exists(args.dts_path) == False or os.path.exists(model_path) == False or os.path.exists(lookup_path) == False:
        print("Some main folder does not exist")
        return
    model_file = os.path.join(model_path, "efm.yaml")
    lookup_file = os.path.join(model_path, "efm_lookups.yaml")
    ...
    for dts_name in DTSs:
        ...
        if os.path.exists(dts_file) == False:
            print("DTS_" + dts_name + ".xlsx file does not exist")
            continue
